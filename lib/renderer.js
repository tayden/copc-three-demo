...
export class Renderer {
  ...
  updatePointSizeDynamic() {
    const fov = this.camera.fov * (Math.PI / 180);
    const viewHeight = 2 * Math.tan(fov / 2) * this.camera.position.distanceTo(this.controls.target);
    const screenHeight = window.innerHeight;

    this.pointsObjects.forEach((points) => {
      const boundingSphere = points.geometry.boundingSphere;
      if (boundingSphere) {
        const projectedSize = screenHeight * boundingSphere.radius / viewHeight;
        const size = Math.max(this.options.minPointSize, Math.min(this.options.maxPointSize, projectedSize));
        points.material.size = size;
      }
    });
  }
